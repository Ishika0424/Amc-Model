...
import { Task, TaskCategory, TaskStatus, Notification, Remark, IncidentReport } from "@/types";
...
interface TaskContextType {
  ...
  addIncidentReport: (incident: Partial<IncidentReport>) => void;
  updateIncidentStatus: (incidentId: string, status: IncidentReport['status'], resolution?: string) => void;
...
export const TaskProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    if (incidentReports.length > 0) {
      mockDataService.saveIncidentReports(incidentReports);
    }
  }, [incidentReports]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const incidentsData = mockDataService.getIncidentReports();
  ...
  const updateTaskStatus = (
    ...
    // If remarks are provided and status is in-progress, create a remark object
    if (remarksText && status === "in-progress") {
  ...
  const addIncidentReport = (incident: Partial<IncidentReport>) => {
    const newIncident: IncidentReport = {
      id: `incident-${Date.now()}`,
      reportedBy: incident.reportedBy || user?.id || "",
  ...
  const updateIncidentStatus = (incidentId: string, status: IncidentReport['status'], resolution?: string) => {
